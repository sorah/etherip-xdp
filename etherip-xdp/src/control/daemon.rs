//! The `etherip-xdp` daemon orchestration: argument parsing, config-directory
//! resolution, and the main run loop that owns the tunnel [`Manager`] and drives
//! reloads, re-resolution, and the embedded management server.
//!
...
/// directories win on a file-name collision (see
/// [`crate::control::config::load_dirs`]).
